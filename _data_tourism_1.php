...
while($baris = pg_fetch_array($hasil)){
	  ...
	  $dataarray[]=array('id'=>$id,'name'=>$name,'address'=>$address,'open'=>$open, 'close'=>$close,'ticket'=>$ticket,'tourism_type'=>$tourism_type,'lng'=>$lng,'lat'=>$lat);
}

	//DATA GALLERY
    $query_gallery="SELECT serial_number, gallery_tourism FROM tourism_gallery where id = '".$cari."' "; 
    $hasil2=pg_query($conn, $query_gallery);
    if($hasil2){
